...
#ifndef SRC_MAIN_CC_MATH_EXPINT_H_
#define SRC_MAIN_CC_MATH_EXPINT_H_

namespace wfa::math {

/// \brief Returns the Euler-Mascheroni constant.
///
...
/// \return the Euler-Mascheroni constant \f$ \gamma = 0.5772156649\ldots \f$
template <class Real>
inline Real euler_gamma() {
  return Real(0.57721566490153286060651209008240243104215933593992l);
}

/// \brief Evaluates std::expint(x) for x < -1.
///
...
/// \brief Evaluates std::expint(x) for x != 0.
///
/// This function evaluates the following expression:
/// \f[
...
/// \brief Evaluates std::expint(x) for x > -std::log(
...
/// \brief Internal implementation of std::expint.
///
